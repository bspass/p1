...
from tkinter import ttk, messagebox  # Импорт ввода текста, и вывода
...
# Функция для проверки, является ли введенный символ числом.
def only_numbers(char):
    return char.isdigit()


def generate_password():  # Определяем функцию для генерации пароля.
    # Проверяем, введена ли длина пароля
    if length_entry.get():
        ...
        if password_characters:
            # Если выбраны символы, генерируем пароль
            password = ''.join(random.choice(password_characters) for _ in range(password_length))
            password_entry.delete(0, tk.END)
            password_entry.insert(0, password)
        else:
            # Если не выбран ни один тип символов, выводим сообщение
            messagebox.showinfo("Ошибка", "Выберите хотя бы один тип символов для пароля.")
    else:
        # Если длина пароля не введена, выводим сообщение
        messagebox.showinfo("Ошибка", "Введите длину пароля.")
...
root = tk.Tk()
root.title("P1")  # Устанавливаем заголовок окна.
